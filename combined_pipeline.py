...
def _fmt_outcomes(lst: list) -> str:
    """Store outcomes as a JSON array of {measure, description, timeFrame} objects.
    This replaces the old formatted-text approach; trial_endpoints mat-view depends on JSON."""
    import json
    if not lst:
        return ""
    return json.dumps([
        {
            ...
            "timeFrame":   (o.get("timeFrame") or "").strip(),
        }
        for o in lst
    ])


# Name fragments that indicate a job title or company rather than a real person
_NON_PERSON_FIRST_WORDS = {
    "medical", "toll", "call", "clinical", "global", "senior", "chief",
    "vice", "executive", "study", "director", "manager", "regulatory",
    "safety", "trial", "project", "country", "regional", "national",
    "local", "site", "head", "pharmacovigilance", "investigational",
    "principal", "lead", "general", "associate", "assistant",
}


def _is_person_name(name: str) -> bool:
    """Return True if name looks like a real person rather than a job title or company."""
    n = name.strip()
    if not n or len(n) < 4:
        return False
    words = n.split()
    if len(words) < 2:
        return False
    if words[0][0].isdigit():          # phone number
        return False
    if words[0].lower().rstrip(',') in _NON_PERSON_FIRST_WORDS:
        return False
    return True
...
def _fmt_officials(lst: list) -> str:
    ...
    for o in lst:
        ...
        affil = (o.get('affiliation') or '').strip()
        if not name or not _is_person_name(name):
            continue
        entry = f"{name} ({role})" if role else name
        if affil:
...
def _fmt_locations(locs: list) -> str:
    ...
    for loc in locs:
        ...
        status   = (loc.get("status") or "").strip().upper()
        addr_parts = [p for p in [city, state, country] if p]
        addr = ", ".join(addr_parts)
        if facility and addr:
            entry = f"{facility} ({addr})"
        elif facility:
            entry = facility
        elif addr:
            entry = f"({addr})"
...
# ── Drug lookup for interventions_normalized ──────────────────────────────────
_drug_lookup: dict[str, str] = {}   # alias.lower() -> canonical drug_name
_drug_lookup_loaded = False

def _load_drug_lookup() -> None:
    global _drug_lookup, _drug_lookup_loaded
    if _drug_lookup_loaded:
        return
    try:
        with db._cur() as cur:
            ...
            for row in cur.fetchall():
                ...
                if not canonical:
                    continue
                _drug_lookup[canonical.lower()] = canonical
                for alias in (row['alias_names'] or '').split('|'):
                    a = alias.strip()
                    if a and a.lower() not in _drug_lookup:
                        _drug_lookup[a.lower()] = canonical
        print(f"  [DrugLookup] Loaded {len(_drug_lookup):,} entries -> {len(set(_drug_lookup.values())):,} canonical names")
    except Exception as e:
        print(f"  [DrugLookup] Warning: could not load drug lookup ({e})")
    _drug_lookup_loaded = True


def _normalize_interventions_list(interventions: list) -> str:
    """Normalize a list of CT.gov intervention dicts to pipe-separated canonical names."""
    _load_drug_lookup()
    seen: set[str] = set()
    result: list[str] = []
    for iv in interventions:
        ...
        if not name:
            continue
        canonical = _drug_lookup.get(name.lower(), name)
...
def _to_bool(val) -> "bool | None":
    """Convert CT.gov boolean values (True/False/None/str) to Python bool or None."""
    if val is None:
        return None
...
def _to_int(val) -> "int | None":
    """Convert a value to int, returning None if empty or non-numeric."""
    if val is None:
        return None
    try:
        return int(val)
...
import re as _re
_DATE_DAY_RE   = _re.compile(r'^\d{4}-\d{2}-\d{2}$')
_DATE_MONTH_RE = _re.compile(r'^\d{4}-\d{2}$')

_MIN_AGE_RE = _re.compile(
    r'^(\d+)\s+(year|years|month|months|week|weeks|day|days|hour|hours)$',
    _re.IGNORECASE
)
_MIN_AGE_UNIT_MAP = {
...
def _parse_min_age(raw: str) -> "tuple[int | None, str | None]":
    """Return (value, unit) from CT.gov minimumAge string e.g. '18 Years' -> (18, 'YEARS')."""
    if not raw:
        return None, None
    m = _MIN_AGE_RE.match(raw.strip())
    if m:
        return int(m.group(1)), _MIN_AGE_UNIT_MAP[m.group(2).lower()]
    return None, None


def _parse_date_pair(raw: str) -> "tuple[str | None, str | None]":
    """Return (iso_date_str, precision) from a CT.gov date string.
    precision is 'DAY', 'MONTH', or None.
    iso_date_str is YYYY-MM-DD (first of month for MONTH precision).
    """
    if not raw:
        return None, None
    raw = raw.strip()
    if _DATE_DAY_RE.match(raw):
        return raw, 'DAY'
    if _DATE_MONTH_RE.match(raw):
        return raw + '-01', 'MONTH'
    return None, None
...
def parse_study_to_organized(nct_id: str, data: dict) -> dict:
    ...
    interventions_normalized = _normalize_interventions_list(interventions_list)
    ...
    return {
        ...
        "Expanded Access Info":        _to_bool(_nest(st, "expandedAccessInfo", "hasExpandedAccess")),
        ...
        # Parsed date companion columns
        **dict(zip(
            ["start_date_parsed", "start_date_precision"],
            _parse_date_pair(_nest(st, "startDateStruct", "date") or "")
        )),
        **dict(zip(
            ["primary_completion_date_parsed", "primary_completion_date_precision"],
            _parse_date_pair(_nest(st, "primaryCompletionDateStruct", "date") or "")
        )),
        **dict(zip(
            ["completion_date_parsed", "completion_date_precision"],
            _parse_date_pair(_nest(st, "completionDateStruct", "date") or "")
        )),
        **dict(zip(
            ["last_update_date_parsed", "last_update_date_precision"],
            _parse_date_pair(_nest(st, "lastUpdatePostDateStruct", "date") or "")
        )),
        **dict(zip(
            ["status_verified_date_parsed", "status_verified_date_precision"],
            _parse_date_pair(_nest(st, "statusVerifiedDate") or "")
        )),
        **dict(zip(
            ["first_post_date_parsed", "first_post_date_precision"],
            _parse_date_pair(_nest(st, "studyFirstPostDateStruct", "date") or "")
        )),
        **dict(zip(
            ["first_submit_date_parsed", "first_submit_date_precision"],
            _parse_date_pair(_nest(st, "studyFirstSubmitDate") or "")
        )),
        ...
        "FDA Regulated Drug":   _to_bool(_nest(ov, "isFdaRegulatedDrug")),
        "FDA Regulated Device": _to_bool(_nest(ov, "isFdaRegulatedDevice")),
        "Has DMC":              _to_bool(_nest(ov, "hasDmc")),
        ...
        "phases":             norm.normalize_phase(design.get("phases", [])),
        ...
        "Enrollment":         _to_int(_nest(design, "enrollmentInfo", "count")),
        "enrollmentInfoType": _nest(design, "enrollmentInfo", "type"),
        "Interventions":               interventions_str,
        "interventions_normalized":    interventions_normalized,
        ...
        **dict(zip(["min_age_value", "min_age_unit"], _parse_min_age(_nest(elig, "minimumAge") or ""))),
        "Standard Ages":       ", ".join(elig.get("stdAges", [])),
        "healthyVolunteers":   _to_bool(_nest(elig, "healthyVolunteers")),
...
def check_etag(stored_etag: str | None) -> tuple[bool, str]:
    ...
    try:
        ...
        print(f"  WARNING: ETag check failed ({exc}) — skipping batch check, using stored etag.")
        return True, stored_etag or ""
...
def _post_run_hook(
    ...
    """
    Phase F-09: After each pipeline run, enqueue jobs into trial_processing_queue
    so that incremental_processor.py can parse field changes and populate history tables.

    - TRIAL_MODIFIED (IMMEDIATE): every NCT with a new field diff this run
    - NEW_TRIAL (NORMAL): newly accepted candidates (added to tracking_list this run)
    """
    jobs: list[dict] = []

    for r in newly_compared:
        nct_id = r.get("nct_id", "")
        try:
            has_changes = int(r.get("field_change_count") or 0) > 0
        except (ValueError, TypeError):
            has_changes = False
        if nct_id and has_changes:
            jobs.append({
                "nct_id":          nct_id,
                "dept":            dept,
                "job_type":        "TRIAL_MODIFIED",
                "priority":        "IMMEDIATE",
                "source_run_date": run_date,
                "payload":         {},
            })

    # new_candidates that already exist in tracking_list are NEW_TRIAL events
    for r in new_candidates:
        nct_id = r.get("NCT ID") or r.get("nct_id", "")
        if nct_id:
            jobs.append({
                "nct_id":          nct_id,
                "dept":            dept,
    ...
    if jobs:
        inserted = db.enqueue_processing_jobs(jobs)
        print(f"  Enqueued {inserted} job(s) ({len([j for j in jobs if j['job_type']=='TRIAL_MODIFIED'])} modified, {len([j for j in jobs if j['job_type']=='NEW_TRIAL'])} new)")
    else:
...
def sync_baseline(nct_ids: list[str], drug_terms: list = (), batch_size: int = 50) -> tuple[dict, list[str]]:
    """
    ...
    Returns (base_dict, newly_inserted_nct_ids) — newly_inserted used by Step 9
    to auto-populate embedding chunks for brand-new trials.
    """
    ...
    if not missing:
        ...
        for i, nct_id in enumerate(missing, 1):
            ...
            if len(raw_batch) >= batch_size:
                new_rows: dict = {}
                _parse_raw_into_base(raw_batch, new_rows)
                base.update(new_rows)
                db.upsert_organized_trials(DEPT_NAME, list(new_rows.values()))
...
def _run_unified(run_start: datetime) -> None:
    ...
    base_organized, newly_inserted_ncts = sync_baseline(all_tracked, drug_compiled)
    ...
    # Snapshot every newly-compared NCT's current version into trial_history
    if newly_compared:
        changed_nct_ids = list({r["nct_id"] for r in newly_compared})
        db.upsert_trial_history_from_version_pairs(DEPT_NAME, changed_nct_ids)
    ...
    # ── Step 8b — Post-run hook: enqueue incremental processing jobs ──────
    print("\n[Step 8b] Enqueueing incremental processing jobs...")
    all_new_candidates = [t for p in pipelines.values() for t in p["new"]]
    _post_run_hook(
        dept           = DEPT_NAME,
        run_date       = to_date,
        newly_compared = newly_compared,
    ...
    # ── Step 9: Concept extraction for new + modified trials ─────────────────
    concept_nct_ids = list({
        *newly_inserted_ncts,
        *(r["nct_id"] for r in newly_compared if int(r.get("field_change_count") or 0) > 0),
    })
    if concept_nct_ids:
        print(f"\n[Step 9] Concept extraction for {len(concept_nct_ids)} trial(s)...")
        import concept_extraction
        concept_extraction.run_for_nct_ids(concept_nct_ids)
    else:
        print("\n[Step 9] Concept extraction — no new or modified trials")

    # Persist unresolved sponsors to DB for ongoing alias maintenance.
    norm.flush_unresolved_to_db(run_date=datetime.now().date())
...
def _run_single(run_start: datetime) -> None:
    ...
    base_organized, newly_inserted_ncts = sync_baseline(nct_ids, drug_compiled)
    ...
    # ── Step 9: Phase F post-run hook — enqueue incremental processing jobs ──────
    print("\n[Step 9] Enqueueing incremental processing jobs...")
    _post_run_hook(
        dept        = DEPT_NAME,
        run_date    = to_date,
        newly_compared   = newly_compared,
    ...
    # ── Step 10: Concept extraction for new + modified trials ─────────────────
    concept_nct_ids = list({
        *newly_inserted_ncts,
        *(r["nct_id"] for r in newly_compared if int(r.get("field_change_count") or 0) > 0),
    })
    if concept_nct_ids:
        print(f"\n[Step 10] Concept extraction for {len(concept_nct_ids)} trial(s)...")
        import concept_extraction
        concept_extraction.run_for_nct_ids(concept_nct_ids)
    else:
        print("\n[Step 10] Concept extraction — no new or modified trials")
    ...
    # ── Embedding (manual) ───────────────────────────────────────────────────
    print("\n[Embedding] Skipped — run embeddings manually: python embedding_worker_v1.py")
