...
# Serialize Python dicts as JSONB automatically
# NOTE: lists are intentionally NOT registered as Json — some queries pass
# plain Python lists as PostgreSQL text[] arrays (e.g. ANY(%s::text[])).
...
_CONCEPT_COLS = frozenset([
    # 21 active Clinical Intelligence concepts (26 original - 7 removed - 1 merged + 3 added)
    "line_of_therapy", "biomarkers", "biomarker_status", "prior_therapies",
    "treatment_status", "treatment_setting", "disease_state",
    "endpoints_normalized", "gene", "genomic_alteration",
    "age_range_normalized", "cns_status", "metastatic_site",
    "organ_function_requirement", "performance_status", "combination_therapy",
    "comparator", "regimen",
...
def _bulk_upsert(
    ...
    preserve_cols: frozenset[str] | None = None,
    batch_size: int = _BATCH,
) -> None:
    """Generic INSERT … ON CONFLICT DO UPDATE for any table.

    preserve_cols: columns to skip in the DO UPDATE clause (keep existing DB value).
    """
    if not records:
        return
    global _conn
    cols        = list(records[0].keys())
    skip        = (conflict_cols if preserve_cols is None else
                   list(conflict_cols) + [c for c in cols if c in preserve_cols])
    update_cols = [c for c in cols if c not in skip]
...
def load_all_indications(dept: str) -> list[str]:
    ...
    with _cur() as cur:
        ...
        return [r["indication"].strip() for r in cur.fetchall() if r["indication"].strip()]
    ...
    def _record(row: dict) -> dict | None:
        ...
        for key, val in row.items():
            if key in ("NCT ID", "last_upserted_at"):
                continue
            # None/bool/int must pass through as-is — _clean(None) → "" which
            # PostgreSQL rejects for BOOLEAN and INTEGER columns
...
# ── Phase F: trial_processing_queue ──────────────────────────────────────────

def enqueue_processing_jobs(jobs: list[dict]) -> int:
    """
    Insert jobs into trial_processing_queue. Skips duplicates (same nct_id + dept + job_type +
    source_run_date that are still PENDING or PROCESSING).
    ...
    records = [
        {
            "nct_id":          _clean(j["nct_id"]),
            "dept":            _clean(j["dept"]),
            "job_type":        _clean(j.get("job_type", "TRIAL_MODIFIED")),
            "priority":        _clean(j.get("priority", "NORMAL")),
            "source_run_date": j.get("source_run_date") or None,
            "payload":         j.get("payload") or {},
        }
        for j in jobs
        if j.get("nct_id") and j.get("dept")
    ]
    if not records:
        return 0
    global _conn
    ...
    col_sql = ", ".join(_qi(c) for c in cols)
    sql = (
        f'INSERT INTO "CT".trial_processing_queue ({col_sql}) VALUES %s '
...
def upsert_trial_history_from_version_pairs(dept: str, nct_ids: list[str]) -> None:
    """Populate trial_history for the given NCTs using data already in nct_version_pairs.
    Called after each pipeline run so trial_history stays current without holding
    curr_full_data in memory during the FLOW 2 loop."""
    if not nct_ids:
        return
    with _cur() as cur:
        cur.execute(
            ...
            INSERT INTO trial_history
                (nct_id, dept, version_num, version_date, overall_status,
                 modules_changed, field_changes, full_data)
            SELECT DISTINCT ON (nvp.nct_id, nvp.dept, nvp.curr_version)
                nvp.nct_id,
                nvp.dept,
                nvp.curr_version,
                nvp.curr_date,
                nvp.curr_status,
                COALESCE(nvp.modules_changed, ''),
                COALESCE(nvp.field_changes,   ''),
                COALESCE(nvp.curr_full_data, '{}')
            FROM nct_version_pairs nvp
            WHERE nvp.dept = %s
              AND nvp.nct_id = ANY(%s::text[])
            ORDER BY nvp.nct_id, nvp.dept, nvp.curr_version, nvp.ctid DESC
            ON CONFLICT (nct_id, dept, version_num) DO NOTHING
            """,
            (dept, list(nct_ids)),
        )
        print(f"  [DB] Trial history upserted: {cur.rowcount} new snapshots for {dept}")
...
def approve_trial(nct_id: str, dept: str, source: str, indication: str = "", decided_by: str = "pipeline"):
    table = "new_candidates_log" if source == "new_candidates" else "unmatched_log"
    today = _date.today().isoformat()
    with _cur() as cur:
        cur.execute(
            f"UPDATE {_qi(table)} SET decision = %s, decided_at = %s, decided_by = %s "
            f"WHERE nct_id = %s AND dept = %s AND indication = %s",
            ("Approved", today, decided_by, nct_id, dept, indication),
        )
